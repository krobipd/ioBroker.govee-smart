...
export class GoveeMqttClient extends ReconnectingMqttClient {
  ...
  protected reconnectExhausted(): boolean {
    return this.probeMode || this.authFailCount >= MQTT_MAX_AUTH_FAILURES;
  }
  ...
  private handleMessage(payload: Buffer, topic: string): void {
    ...
    try {
      ...
      // no local API had no reachability evidence at all even though Govee kept
      // telling us (issues #22/#25/#26, #31, #18). The envelope's `pactType`
      // was forwarded alongside it and never read by anything —
      // `readReportedReachability` decides on the field shape, because the
