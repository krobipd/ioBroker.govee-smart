...
`resolveDeviceReachability(device, now)` (`device-manager/lookups.ts`) beantwortet „ist das Gerät erreichbar" für **jede** Geräteart. Es gibt genau vier Stufen, in dieser Reihenfolge: **LAN-entschieden** (`isLanDriven`) → die LAN-Antwortfrische (`LAN_REPLY_FRESHNESS_MS` 90 s), sonst nichts. **Gateway tot** (`state.gatewayOnline === false`) → nicht erreichbar, Obergrenze. **Govee hat gemeldet, FRISCH** (`state.cloudReportedOnline` + `cloudReportedOnlineAt` jünger als `CLOUD_ONLINE_EVIDENCE_TTL_MS`, 30 min) → sein Wort, in beide Richtungen. **Sonst** → nicht erreichbar. **`isLanDriven` hängt dabei NICHT an `lanIp`.** Die Adresse wird bei jedem Start neu per Suchlauf gefunden (`cache.ts:60,93` verwirft sie absichtlich), also sieht direkt nach dem Start jedes Licht adresslos aus. Es an `lanIp` festzumachen würde jedes lokale Licht für eine Runde dem Cloud-Zwischenspeicher überlassen — genau der 2.29.0-Fehler. Stattdessen entscheidet `lastLanSeenAt`: persistiert, gesetzt an jeder LAN-Antwort, gültig für `LAN_CAPABLE_MEMORY_MS` (7 Tage). Der Verfall ist die Gegenrichtung: wer die lokale Schnittstelle in der App abschaltet, wandert nach einer Woche auf den Cloud-Weg statt für immer grau zu bleiben. **Der Konto-Push liefert die Erreichbarkeit — seit 2.30.0 wird sie auch gelesen.** Eigener Pakettyp `cmd:"online"`, die Auskunft in `state.connected` (Text) oder `state.result` (Zahl). **Falle:** `result` steht auch in `status`/`ptReal`, dort ist es ein Ergebnis-Code — nur im `online`-Paket ist es Erreichbarkeit. Es entscheidet die Feldform (`readReportedReachability`), NICHT `pactType` (die Kontoliste führt Geräte ohne `pactType`). Unbekannter Wert = kein Beweis, nie ein Ausweichen auf „erreichbar".

**Reihenfolge, tragend:** eine ausdrückliche Meldung schlägt die bloße Ankunft eines Pakets — sonst zählt Govees eigenes „dieses Gerät ist weg" als Lebenszeichen (bis 2.30.0 genau so bei Haushaltsgeräten). **Der 2-Minuten-Konto-Abruf ist der garantierte Erneuerer** und läuft seit 2.30.0 auch für Lichter ohne lokale Schnittstelle (`hasDeviceNeedingAppApi`): der Push ist ereignisgetrieben, ein Gerät das einfach dasteht sendet nichts, und ohne den Abruf verfiele sein Beweis. **Gateway-Geräte** (Batterie-Sensoren/-Taster) haben keine eigene Verbindung. Das GATEWAY ist ihr Erreichbarkeits-Gerät (krobi 2026-09-03): `gatewayInfo.device` verknüpft, `resolveGatewayReachability()` löst auf. Bewusst nur **Obergrenze** — totes Gateway = unerreichbar, lebendes ≠ erreichbar (leere Batterie/außer Reichweite wäre sonst falsches Grün). Positiver Beweis bleibt der eigene frische Messwert (`isSensorDataFresh`).
