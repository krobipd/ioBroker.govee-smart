...
`resolveDeviceReachability(device, now)` (`device-manager/lookups.ts`) beantwortet „ist das Gerät erreichbar" für **jede** Geräteart, und **jeder** Konsument fragt sie — der Objektbaum, die Geräteliste im Reiter Experte, die Assistenten-Sperre, `groups.info.membersUnreachable`, die Kopfzahl. Bis 2.31.2 lasen vier davon stattdessen `state.online`, das nur für Lichter und nur bei bewiesener Antwort zurückgeschrieben wird: die Admin zeigte grün, was der Baum grau zeigte, und der Assistent startete darauf. Es gibt fünf Stufen, in dieser Reihenfolge: **LAN-entschieden** (`isLanDriven`) → die LAN-Antwortfrische (`LAN_REPLY_FRESHNESS_MS` 90 s), sonst nichts. **Gateway tot** (`state.gatewayOnline === false`) → nicht erreichbar, Obergrenze. **Govee hat AUSDRÜCKLICH gemeldet, FRISCH** (`state.cloudReportedOnline` + `cloudReportedOnlineAt` jünger als `CLOUD_ONLINE_EVIDENCE_TTL_MS`, 30 min) → sein Wort, in beide Richtungen. **Govee hat etwas GELIEFERT** (`state.cloudLivenessAt`, gleiche Frist) → das Paket existiert nur, weil das Gerät gesprochen hat: Beweis, aber nur nach oben, und es verliert gegen eine Meldung in JEDE Richtung. **Sonst** → nicht erreichbar. **`isLanDriven` hängt dabei NICHT an `lanIp`.** Die Adresse wird bei jedem Start neu per Suchlauf gefunden (`cache.ts:60,93` verwirft sie absichtlich), also sieht direkt nach dem Start jedes Licht adresslos aus. Es an `lanIp` festzumachen würde jedes lokale Licht für eine Runde dem Cloud-Zwischenspeicher überlassen — genau der 2.29.0-Fehler. Stattdessen entscheidet `lastLanSeenAt`: persistiert, gesetzt an jeder LAN-Antwort, gültig für `LAN_CAPABLE_MEMORY_MS` (7 Tage). Der Verfall ist die Gegenrichtung: wer die lokale Schnittstelle in der App abschaltet, wandert nach einer Woche auf den Cloud-Weg statt für immer grau zu bleiben. **Der Konto-Push liefert die Erreichbarkeit — seit 2.30.0 wird sie auch gelesen.** Eigener Pakettyp `cmd:"online"`, die Auskunft in `state.connected` (Text) oder `state.result` (Zahl). **Falle:** `result` steht auch in `status`/`ptReal`, dort ist es ein Ergebnis-Code — nur im `online`-Paket ist es Erreichbarkeit. Es entscheidet die Feldform (`readReportedReachability`), NICHT `pactType` — die Kontoliste führt Geräte ohne `pactType`. Das Feld wurde bis 2.31.2 aus jedem Umschlag geparst und weitergereicht, ohne dass irgendetwas es las; es ist entfernt, die Beobachtung steht an den beiden Feldern, die sie erklärt. Unbekannter Wert = kein Beweis, nie ein Ausweichen auf „erreichbar".

**Reihenfolge, tragend:** eine ausdrückliche Meldung schlägt die bloße Ankunft eines Pakets — sonst zählt Govees eigenes „dieses Gerät ist weg" als Lebenszeichen. Bis 2.31.2 landeten BEIDE im selben Feld, also gewann der letzte Schreiber: ein Ereignis Sekunden nach einem gemeldeten `online:false` ersetzte dieses `false` eine halbe Stunde lang durch ein erfundenes `true`. Die Regel stand seit 2.30.0 hier und war nicht umgesetzt; getrennte Felder (`cloudReportedOnline*` = gehört, `cloudLivenessAt` = angekommen) sind, was sie durchsetzt.

**Erneuerer, nach Zugangsdaten-Stufe.** Mit Konto: der Push (ereignisgetrieben) und der 2-Minuten-Konto-Abruf, der seit 2.30.0 auch für Lichter ohne lokale Schnittstelle läuft (`hasDeviceNeedingAppApi`) — ein Gerät, das einfach dasteht, sendet nichts. **Beide brauchen E-Mail + Passwort**, und `pollAppApi` bricht ohne Bearer-Token sofort ab. Auf der Stufe „nur API-Key" erneuerte deshalb bis 2.31.2 **nichts**: 30 Minuten nach dem Start wurde ein Licht ohne lokale Schnittstelle grau und blieb grau, während es die ganze Zeit Befehle annahm (#18–#26). `DeviceManager.refreshExpiringReachability()` schließt das — ein gezielter `/device/state`-Abruf für genau die Geräte, deren Beweis älter als `CLOUD_REACHABILITY_REFRESH_MS` (20 min) ist, mit nichts als dem API-Key, in der Warteschlange hinter Befehl und Zustands-Abruf, höchstens ein Aufruf je Gerät und 5 Minuten. Verdrahtet in `main.ts` am selben 2-Minuten-Takt, aber **über** `pollAppApi` — darin läge er hinter genau dem Abbruch, den er umgehen soll. Eine Anlage, deren Beweise frisch bleiben, verursacht keinen zusätzlichen Aufruf. **Gateway-Geräte** (Batterie-Sensoren/-Taster) haben keine eigene Verbindung. Das GATEWAY ist ihr Erreichbarkeits-Gerät (krobi 2026-09-03): `gatewayInfo.device` verknüpft, `resolveGatewayReachability()` löst auf. Bewusst nur **Obergrenze** — totes Gateway = unerreichbar, lebendes ≠ erreichbar (leere Batterie/außer Reichweite wäre sonst falsches Grün). Positiver Beweis bleibt der eigene frische Messwert (`isSensorDataFresh`).

**`proven` (zweiter Rückgabewert):** eine GEHÖRTE Erreichbarkeit darf in beide Richtungen ins Gerät zurückgeschrieben werden, eine unbewiesene nie — ein unbewiesenes `false` war der ursprüngliche Dauerdefekt. **`info.connection` folgt einer ANDEREN Regel** (v2.13.0-Vertrag): „arbeitet der Adapter", nicht „ist das Gerät da"; `groups.info.online` ebenso = Cloud-Verbindung (i18n `cloudOnline`).
...
- **Ablage:** Meta-Objekt `<namespace>.diagnostics` (Bauart wie `snapshots`), `diag.lastExport` hält seit 2.31.0 den ZEITPUNKT des letzten Berichts (ISO, UTC, `role: "date"`, Typ bleibt `string`). `diag.result` (2.29.0) und der Knopf `diag.export` (2.31.0) sind weg — beide ausdrücklich in `migrateLegacyDiagnostics` entfernt, weil `cleanupCloudOwnedStates` nur MANAGED_CHANNELS fegt und `diag` keiner ist. **Der Zeitpunkt braucht dort auch einen Wert-Reset:** der Typ blieb `string`, und `writeStateDefsToChannels` schreibt den Vorgabewert nur in einen LEEREN Zustand — ohne Reset überlebt der alte Dateiname unter dem neuen Namen. `DIAGNOSTICS_KEEP_PER_DEVICE` = 3.
...
- **Admin-Karte „Diagnose"** (`src-admin/DiagnosticsPanel`, im Reiter Experte): Gerät wählen, ein Knopf, Browser-Download. Liste bewusst UNGEFILTERT — gebraucht wird der Bericht, wenn ein Gerät klemmt. **Seit 2.31.0 der einzige Weg zum Bericht.**
- **Eine Geräteliste für beide Werkzeuge** (`diagnostics {action:"list"}`): liefert je Gerät `online` + `segments`, der Assistent filtert in der Karte (`segmentCapable`). `getSegmentDevices` ist weg — die Schlüssel waren ohnehin identisch. **Kein Zwischenspeicher über den Umschalter:** `online` ist Live-Zustand, deshalb ist immer nur EIN Werkzeug montiert und die Liste wird bei jedem Wechsel neu geholt. **Ein Fehlschlag wirft** (`DeviceListError`) — bis 2.30.0 wurde er zur leeren Liste und die Karte meldete „keine Geräte" für zehn Geräte. Der Rumpf verlässt sich nicht auf den Filter: `SegmentWizard.start()` weist ein unerreichbares Gerät seit 2.31.0 selbst ab.

## Cloud REST API v2

**Base URL:** `https://openapi.api.govee.com` · **Auth:** Header `Govee-API-Key: <key>`

- Rate Limits: 10/min/Gerät, 10.000/Tag — **Appliances aber nur 100/Tag (!)**. Der `rate-limiter` deckt beides ab: die Minute über ein globales Dach von 8 (immer unter den 10/min eines einzelnen Geräts), den Tag zweigleisig — 9.000 global PLUS ein eigenes Tagesbudget je Appliance (90). Ein ERSCHÖPFTES Gerätebudget stellt nicht in die Warteschlange: es setzt sich erst beim Tageswechsel bei Govee zurück, ein wartender Befehl liefe also Stunden später los. Ein Aufruf, der wegen des Minutenlimits wartet, trägt sein Budget aber MIT — bis 2.31.2 buchten nur die Sofortpfade, also lief jeder eingereihte Aufruf ungezählt durch und der Schutz versagte genau unter Last. Gebucht wird beim Ausführen, und die Erlaubnis wird dort erneut geprüft (in der Zwischenzeit können Sofortaufrufe den Tag des Geräts aufgebraucht haben). Eigentümer der Regel ist `rate-limiter.ts:applianceBudget` — der Zustands-Abruf reicht es seit 2.31.2 ebenfalls mit, was sein eigener Kommentar schon immer behauptete. Warum überhaupt nötig: Appliance-Steuerung hat keinen lokalen Weg, jede Schreiboperation ist ein Cloud-Aufruf, und ohne Gerätekonto könnte EIN Gerät die 9.000 des Kontos verbrauchen. Der Adapter selbst tut das nie (kein periodischer Geräte-Poll) — ein Skript schon.
...
- **Diagnostics Export:** Reiter Experte → Diagnose erzeugt strukturiertes JSON (Capabilities, Szenen, Libraries, Quirks, State, recentLogs, lastMqttPackets, runtimeState) für GitHub-Issues. Der Erreichbarkeits-Abschnitt entscheidet NICHTS selbst: `resolveDeviceReachability` liefert `decidedBy` + `lastEvidenceAt` mit, der Bericht bildet sie nur ab — die frühere handgeschriebene Zweitfassung war 2.30.0 in allen vier Angaben falsch. Die beiden verbliebenen Handfassungen (`silentSources`/`refreshedBy`) waren 2.31.1 erneut abgedriftet — sie nannten den Konto-Push als geschlossene Quelle für Cloud-Lichter, wo er seit 2.30.0 gelesen wird, und versprachen einen Erneuerer, den die API-Key-Stufe nicht hat. Sie leiten sich jetzt aus derselben Fallunterscheidung ab und **nennen zu jedem Erneuerer die nötigen Zugangsdaten** — das ist der Unterschied zwischen „wird aufgefrischt" und „verfällt in 30 Minuten". Der Segment-Abschnitt trägt beide Zahlen (physische Länge UND Baumgröße); sie weichen genau im Fall des geschnittenen Streifens voneinander ab.
...
`admin/i18n/*.json`: 135 Schlüssel × 11 Sprachen. **Zweiter Konsument:** `Entwicklung/scripts/sync-iopackage-from-i18n.py` (Release-Gate) schreibt die `common.name`/`desc` der `instanceObjects` aus den Keys `information`, `infoConnection`, `infoMqttConnected`, `infoCloudConnected`, `infoOpenapiMqttConnected(Desc)`, `devicesFolder`, `groups`, `localSnapshotsFolder`, `infoManualSyncDevices(Desc)`. Ein grep über `src/` + `admin/` findet sie NICHT — sie sind trotzdem in Gebrauch (2.28.0: versehentlich entfernt, Gate hat es gefangen). Wird ein `instanceObjects`-Datenpunkt umbenannt, muss die Zuordnung im Skript mitziehen. Die Konto-E-Mail wird **getrimmt** verwendet — beim Karten-Test UND beim Start-Login (`accountEmail` in `onReady`); das Passwort bleibt roh (Leerzeichen können dazugehören).
