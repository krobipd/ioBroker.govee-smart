...
> ✅ **DONE** (two commits). **Removal (commit A):** dropped `info.wizardStatus` (onReady state-init, the `runWizardStep` status mirror, the io-package instanceObject) + a one-shot `delObjectAsync` orphan cleanup; removed `getStatusText`/`wizardIdleText` (only the mirror used them) + the `WizardHandlerAdapter.setState` seam; swept **17 dead i18n keys × 11 langs** (6 getStatusText-only + 11 old jsonConfig sendTo-button keys), parity preserved. The central `sync-iopackage-from-i18n.py` had a govee `info.wizardStatus→infoWizardStatus` mapping that then failed the gate → removed it (committed to the `Entwicklung` tooling repo). Kept the backend `message`/`progress` builders (the existing backend tests pin `r.progress`/`r.message`, so rewriting them is a separate task). **i18n completion (commit B):** added the 9 remaining component langs (es/fr/it/nl/pl/pt/ru/uk/zh-cn) — terminology mined from the deleted 11-lang wizard keys; self-verified 18-key parity + `%s` placeholder counts (component i18n has no auto gate). **Gates:** tsc, 1210 unit, lint 0/0, build, sync-iopackage in-sync, **integration (adapter starts clean — the v2.20.0 onReady scar)**, MF build (11 langs in admin/custom/i18n).
