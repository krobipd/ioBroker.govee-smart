...
User-Doku im [Wiki](https://github.com/krobipd/ioBroker.govee-smart/wiki) ist bilingual (DE + EN). Die Geräte-Seite wird via `npm run gen-wiki` aus `devices.json` generiert — manuelle Edits an `Geraete.md` / `Devices.md` werden überschrieben. **Alle anderen Wiki-Seiten** (Setup, Behavior, Sensors-and-Appliances, Sidebar, Footer …) werden von Hand gepflegt und nicht automatisch regeneriert.

User-facing docs in the [Wiki](https://github.com/krobipd/ioBroker.govee-smart/wiki) are bilingual. The devices page is generated via `npm run gen-wiki` from `devices.json` — manual edits to `Geraete.md` / `Devices.md` get overwritten. **Every other wiki page** (Setup, Behavior, Sensors-and-Appliances, Sidebar, Footer …) is hand-maintained and not regenerated by the script.
