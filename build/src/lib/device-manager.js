...
class DeviceManager {
                ...
                if (cd.type === "light" ||
                    ...
                    if (device) {
                        ...
                        const loadScenes = async () => {
                            try {
                                ...
                                if (lightScenes.length > 0 ||
                                    diyScenes.length > 0 ||
                                    snapshots.length > 0) {
                                    const scenesChanged = lightScenes.length !== device.scenes.length ||
                        ...
                        if (device.snapshots.length === 0) {
                            const snapCap = cd.capabilities.find((c) => c.type === "devices.capabilities.dynamic_scene" &&
                                c.instance === "snapshot" &&
                        ...
                        if (device.sceneLibrary.length === 0 && this.mqttClient?.token) {
                            try {
                                ...
                                if (lib.length > 0) {
                                    ...
                                    changed = true;
                                    this.log.debug(`Scene library for ${cd.sku}: ${lib.length} scenes`);
                                }
                            }
                            catch {
    ...
    handleLanDiscovery(lanDevice) {
        ...
        for (const dev of this.devices.values()) {
            if ((0, types_js_1.normalizeDeviceId)(dev.deviceId) === (0, types_js_1.normalizeDeviceId)(lanDevice.device)) {
                matched = dev;
                break;
            }
        ...
        if (matched) {
            ...
            const shortId = (0, types_js_1.normalizeDeviceId)(lanDevice.device).slice(-4);
            const device = {
                sku: lanDevice.sku,
            ...
            this.log.debug(`LAN: New device ${lanDevice.sku} at ${lanDevice.ip} (no Cloud data)`);
            this.onDeviceListChanged?.(this.getDevices());
        }
    }
    /**
    ...
    async sendCapabilityCommand(device, capabilityType, capabilityInstance, value) {
        ...
        const execute = async () => {
            await this.cloudClient.controlDevice(device.sku, device.deviceId, capabilityType, capabilityInstance, cloudValue);
        };
        if (this.rateLimiter) {
    ...
    async sendSegmentBatch(device, commandStr) {
        ...
        if (parsed.color !== undefined) {
            const execute = async () => {
                await this.cloudClient.controlDevice(device.sku, device.deviceId, cap.type, cap.instance, { segment: parsed.segments, rgb: parsed.color });
            };
            if (this.rateLimiter) {
                await this.rateLimiter.tryExecute(execute, 0);
            }
        ...
        if (parsed.brightness !== undefined) {
            const brightCap = device.capabilities.find((c) => c.type.includes("segment_color_setting") &&
                c.instance.toLowerCase().includes("brightness"));
            const execute = async () => {
                await this.cloudClient.controlDevice(device.sku, device.deviceId, (brightCap ?? cap).type, (brightCap ?? cap).instance, { segment: parsed.segments, brightness: parsed.brightness });
            };
            if (this.rateLimiter) {
                await this.rateLimiter.tryExecute(execute, 0);
            }
    ...
    sendMqttCommand(device, command, value) {
        ...
        switch (command) {
            ...
            case "colorTemperature":
                return this.mqttClient.setColorTemperature(device.deviceId, value);
            default:
                return false;
        }
    }
    ...
    findDeviceBySkuAndId(sku, deviceId) {
        ...
        // Normalized search
        const normalizedId = (0, types_js_1.normalizeDeviceId)(deviceId);
        for (const dev of this.devices.values()) {
            if (dev.sku === sku && (0, types_js_1.normalizeDeviceId)(dev.deviceId) === normalizedId) {
                return dev;
            }
        }
        return undefined;
    ...
    deviceKey(sku, deviceId) {
        return `${sku}_${(0, types_js_1.normalizeDeviceId)(deviceId)}`;
    }
    /**
    ...
    logDedup(context, err) {
        const category = (0, types_js_1.classifyError)(err);
